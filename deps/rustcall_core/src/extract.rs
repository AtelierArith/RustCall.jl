...
impl TreeScan {
    ...
    /// Scan with the target crate's Rust edition. This matters for a leading
    /// `::` path, which changed from crate-rooted to extern-rooted in 2018.
    pub fn with_edition(edition: &str) -> Self {
        Self {
            julia: CrateScan::default(),
            pyo3: crate::pyo3::Pyo3Scan::with_edition(edition),
        }
    }

    /// Scan generated includes using the target's Cargo build environment.
    /// With this context, unresolved include expressions are errors instead
    /// of silently omitting an API generated by the build script.
    pub fn with_include_environment(environment: crate::include_paths::IncludeEnvironment) -> Self {
        Self {
            julia: CrateScan {
                include_environment: Some(environment),
                ..CrateScan::default()
            },
            pyo3: crate::pyo3::Pyo3Scan::default(),
        }
    }

    pub fn with_include_environment_and_edition(
        environment: crate::include_paths::IncludeEnvironment,
        edition: &str,
    ) -> Self {
        Self {
            julia: CrateScan {
...
pub struct CrateScan {
    include_environment: Option<crate::include_paths::IncludeEnvironment>,
        ...
        for item in items {
            match item {
                ...
                Item::Macro(m) if m.mac.path.is_ident("include") => {
                    ...
                    // wraps them (#315 review). Generated paths are resolved
                    // against explicit target build context when supplied.
                    // Reading it is the caller's job (#343), which is
                    ...
                    let path = match &self.include_environment {
                        Some(environment) => environment
                            .resolve(m.mac.tokens.clone())
                            .map_err(ExtractError::Unsupported)?,
                        None => {
