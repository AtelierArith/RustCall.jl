...
fn render(krate: &Ident, items: TokenStream2, uses_user_crate: bool) -> String {
    ...
    // The generated crate is a whole file RustCall writes, so it carries the
    // quiet panic hook its wrappers take a boundary guard from (#304).
    let hook = crate::codegen::panic_hook_items();
    let file: syn::File = syn::parse2(quote! { #glob #items #hook })
        .expect("generated wrapper crate is not valid Rust");
