...
### Sources generated by Cargo build scripts

The resolved PyO3 scan runs the target's build script through Cargo before
following generated includes. It uses that target package's `OUT_DIR` and
`cargo:rustc-env` values, not a dependency's build environment or an ambient
`OUT_DIR` from Julia's process. For example, a crate can expose its entire API
through `include!(concat!(env!("OUT_DIR"), "/api.rs"))`, even when its committed
sources contain no PyO3 markers. Custom `[package] build` paths are honored by
Cargo; `build = false` does not trigger build-script discovery.

Include paths support string literals, nested `concat!` of strings, and
`env!` names present in the explicit build context. This is not arbitrary
macro expansion. An unsupported path expression or a missing environment
value in a resolved include is an extraction error, including with
`skip_unparsable = true`. A plain `scan_crate` without a resolved build context
still sees only the source it can follow without running a build script.

The cfg probe and actual wrapper build share their dependency output directory,
so a generated API that depends on its `OUT_DIR` is scanned at the path the
build uses. The files actually read by extraction enter the wrapper's artifact
identity and generated module inputs. Changed generated contents invalidate
the wrapper cache even when the build script's input lies outside the crate;
previously loaded wrappers keep their own generation. Both `@rust_crate` and
`write_bindings_to_file` use this resolved path.
...
* `cargo tree -e features,normal -p <target>` runs in a generated wrapper-shaped
  edition-2021 root and gives the **resolved** feature set of the target package
  and pyo3 — transitive closure, target tables and renames already applied.
  Requested features belong to the target dependency, not the wrapper itself.
  The target workspace's lockfile and root overrides are carried into this
  project. Both the root and edge filter matter: a resolver-1 target can unify
  pyo3's normal and build-dependency features when built on its own, although
  the wrapper's resolver keeps them separate. Build-side `extension-module`
  must not make that wrapper unlinkable; unrelated workspace members must not
  activate features in its graph either.
