...
- **The proc macro refuses a generic `#[julia]` item** (#462): a type or `const` parameter, or `impl Trait` in a signature, on a function, struct, impl block or method is one spanned `compile_error!` at the item (`generic_item_error` in `codegen.rs`, trybuild `tests/ui/generic_*.rs`); lifetime parameters are wrapped as usual. Generics are a `rust"""` feature, monomorphized through `specialize` — for free functions and generic structs only: a generic `pub fn` of a **non-generic** inline struct is refused at the method too (`inline_generic_method_error`, #471), gated by its `#[cfg]` and left out of the manifest, since no struct parameter binds the method's own.
