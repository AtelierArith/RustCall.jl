...
- `src/crate_bindings.jl` — composition root for crate bindings, included into the existing `RustCall` module. `crate_scan.jl` owns crate metadata, scanning and Rust wrapper projects; `crate_build_env.jl` owns build records and environment verification; `crate_layout.jl` owns the module tree and shared naming; `crate_module_expr.jl` / `crate_wrappers_expr.jl` emit in-memory modules and wrappers (including shared field/payload plans); `crate_module_source.jl` emits written source; `crate_build.jl` orchestrates builds and defines artifact keys and format compatibility; `crate_runtime.jl` owns binding proxies and `@rust_crate`; `crate_write.jl` orchestrates persisted bindings. Keep the include order in the composition root, and keep these files beside it so native source-relative paths retain their meaning. Source-level tests use `test/source_helpers.jl` to follow the component includes; a move must not silently narrow their coverage.
