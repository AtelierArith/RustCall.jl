...
- `src/native_layout.jl` — the one place that decides where `Pkg.build` puts the two native products and where they are found again (#258). Included by `src/RustCall.jl` **and** by `deps/build.jl`, which runs before the module exists, so the build and the lookup cannot drift. A checkout builds into `deps/<crate>/target`; an installed package (a tree under a depot's `packages/`) builds into `<depot>/scratchspaces/<UUID>/native-v1/<slug>/<crate>` and its package directory is never written to. `deps/build.jl` runs no `cargo clean`: Cargo's own fingerprint decides what to redo. Overrides: `RUSTCALL_EXTRACT`, `RUSTCALL_RUST_HELPERS`.
