...
- `src/manifest.jl` — runs the CLI, validates `schema_version`, converts the TOML manifest into `RustFunctionSignature` / `RustStructInfo` / `RustMethod`, and computes `toolchain_fingerprint()` (the schema identifier + the sources of `rustcall_core` / `rustcall_julia_macros` / `rustcall_extract` with each `[package] version` left out + `artifact_compiler_identity()`) that is part of every cache key. The extractor *binary* is deliberately not in it (#372): a patch release bumps the crate version and Cargo folds that into `-C metadata`, so the same sources give a byte-different executable, and the key would move on a release that promises to keep it.
