...
- Publishing is automated: `.github/workflows/PublishCrates.yml` runs `scripts/publish_rust_crates.sh` after a green `CI` on `main`, publishing whichever of the three is not on crates.io yet, in dependency order. The script is idempotent and has a `--dry-run`; the `CARGO_REGISTRY_TOKEN` secret lives in the `release` environment and must never reach a pull request.
