...
@testset "Module-qualified symbols (#300)" begin
    ...
    @testset "the Julia layout mirrors the Rust module tree" begin
        mktempdir() do dir
            ...
            @test occursin("_call_target(_TC_fn_rustcall_a__run, \"rustcall_a__run\")", text)
            @test occursin("_call_target(_TC_fn_rustcall_b__run, \"rustcall_b__run\")", text)
            @test occursin("_call_target(_TC_fn_rustcall_run, \"rustcall_run\")", text)
            @test occursin("_ctor_target(_TC_m_rustcall_a__C_new, \"rustcall_a__C_new\", \"a__C_free\")", text)
            @test occursin("_ctor_target(_TC_m_rustcall_b__C_new, \"rustcall_b__C_new\", \"b__C_free\")", text)
            ...
            @test occursin("_call_target(_TC_fn_rustcall_a__deep_0er__snake_0case_0fn, \"rustcall_a__deep_0er__snake_0case_0fn\")", code)
            @test occursin("_ctor_target(_TC_m_rustcall_a__C_new, \"rustcall_a__C_new\", \"a__C_free\")", code)
            @test occursin("_struct_generation(_TC_free_b__C_free, \"b__C_free\")", code)
    ...
    if !_MS_HAVE_CARGO || !RustCall.check_rustc_available()
        ...
        @testset "a module written by write_bindings_to_file binds both `run`s" begin
            mktempdir() do dir
                ...
                @test occursin("\nmodule a\n", content)
                @test occursin("\nmodule b\n", content)
                @test occursin("_call_target(_TC_fn_rustcall_a__run, \"rustcall_a__run\")", content)
                @test occursin("_ctor_target(_TC_m_rustcall_b__C_new, \"rustcall_b__C_new\", \"b__C_free\")", content)
