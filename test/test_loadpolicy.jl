...
@testset verbose=true "LoadPolicy" begin
    ...
    @testset "panic: pinned unwind and a catching boundary (#244)" begin
        ...
        @test occursin("cmd = setenv(`\$cargo_cmd \$build_args`, build_env; dir = String(working_directory))",
                       _src("cargobuild.jl"))
    ...
    @testset "hot reload rebuilds before it swaps (#255)" begin
        ...
        scan_at = findfirst("_scan_crate_signatures(record; env = env)", src)
        build_at = findfirst("rebuild_crate(record; env = env)", src)
