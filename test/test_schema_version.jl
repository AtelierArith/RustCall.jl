...
@testset "manifest schema identifier follows the release (#372)" begin
    ...
    @testset "the published crates share one version" begin
        ...
        # ...and each pins the crate below it to exactly that version: the
        # facade's checked-in `rt.rs` is generated by the core of the same
        ...
        for (crate, below) in (("rustcall_julia_macros", "rustcall_julia_macros_impl"),
            ...
            toml = TOML.parsefile(joinpath(_ROOT, "deps", crate, "Cargo.toml"))
            spec = toml["dependencies"][below]
            @test spec["path"] == "../" * below
            @test spec["version"] == "=" * string(shared)
        end
    end
