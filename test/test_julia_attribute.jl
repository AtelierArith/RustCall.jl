...
@testset "Julia Attribute Support" begin
    ...
    @testset "manifest: schema version guard" begin
        ...
        # #276), schema 4 predates the PyO3 scan columns (`attribute` origins,
        # `vis`, `skip_reason`, `py_result`, #275) and schema 5 predates the
        # PyO3 *wrapper* columns (an exported `py_*` entry with a `return_abi`,
        # the `i32` `err_type` of a lowered `PyResult`, and the generator's own
        # skip reasons, #275 Phase 2); a consumer must not fall back to any of
        # them.
        err = try
