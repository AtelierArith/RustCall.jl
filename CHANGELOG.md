...
- **A call cannot straddle a hot reload.** Every FFI entry point resolves what
  ...
  struct destructor, liveness flag — in one locked step and then uses only that
  snapshot, so a library replaced mid-call can no longer have the call enter
  the retired image while the `free`, or the panic channel, belongs to its
  replacement. In practice this fixes a reload racing a call that returns a
  `String` (the buffer was released through the wrong image's allocator) and a
  reload racing a struct construction (the object could capture one
  generation's destructor and another's liveness flag).
