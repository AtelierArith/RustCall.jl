...
builds, `:bindings_file` for a file `write_bindings_to_file` wrote. Each
emitter's `__init__` names its own (`_crate_init_prologue`).
...
# Every emitter names its module's `origin` (`_crate_init_prologue`). The call
# that names none is a file `write_bindings_to_file` wrote before #531: an
# in-memory `@rust_crate` module is always emitted by the RustCall that loads
# it (Julia re-precompiles a package whose RustCall changed), so only a written
# file can still carry the older call.
...
with `origin = :rust_crate` and `emit_crate_module_code` prints them with
`origin = :bindings_file`, so a written file cannot drift from `@rust_crate`
(#474 review) and each names the remedy that fits it (#531). First the strict
...
function _generate_py_result_method_wrapper(info::RustStructInfo, method::RustMethod,
    ...
    ok_value = if is_unit
        :nothing
    elseif boxed
        :($struct_name($c_sym.ok_value, $free_sym, $release_alive, $free_channel_sym))
    else
        :(_result_payload($ok_julia_type, $c_sym.ok_value, $free_expr))
    end
...
function emit_crate_module_code(info::CrateInfo, lib_path::String;
    ...
    # Through RustCall, as the in-memory module does (#339): a written file is
    ...
    push!(lines, "    rustcall_process_image = RustCall.loadable_library_copy(_LIB_PATH)")
    push!(lines, "    _PIN_LIBRARY && RustCall.preload_dependency!(RustCall.crate_direct_policy(), rustcall_process_image)")
    push!(lines, "    RustCall.load_artifact!(RustCall.crate_direct_policy(), rustcall_process_image;")
    if isempty(preload)
...
function _emit_py_result_method_code(info::RustStructInfo, method::RustMethod,
    ...
    free_var = _generated_local("free_ptr", method.arg_names)
    alive_var = _generated_local("alive", method.arg_names)
    release_alive = _python_owned_handle(info) ? "Ref(true)" : string(alive_var)
    free_channel_var = _generated_local("free_panic_channel", method.arg_names)
