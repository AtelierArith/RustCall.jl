...
function _generate_result_function_wrapper(func::RustFunctionSignature, result_info::ResultTypeInfo, arg_syms::Vector{Symbol}, converted_args::Vector)
    ...
    quote
        ...
        function $func_name($(arg_syms...))
            ...
            if c_result.is_ok == 1
                LastCall.RustResult{$ok_julia_type, $err_julia_type}(true, c_result.ok_value)
            else
                LastCall.RustResult{$ok_julia_type, $err_julia_type}(false, c_result.err_value)
            end
        end
        export $func_name
    end
    ...
    quote
        ...
        function $func_name($(arg_syms...))
            ...
            if c_option.is_some == 1
                LastCall.RustOption{$inner_julia_type}(true, c_option.value)
            else
                LastCall.RustOption{$inner_julia_type}(false, nothing)
            end
        end
        export $func_name
    end
