...
# The build-environment policy — `ARTIFACT_BUILD_ENV_PREFIXES`,
# `ARTIFACT_BUILD_ENV_NAMES`, `ARTIFACT_BUILD_ENV_DENY_SUBSTRINGS`,
# `ARTIFACT_BUILD_SCRIPT_ENV_NAMES`, `ARTIFACT_BUILD_SCRIPT_ENV_PREFIXES` and
# `artifact_build_env_captured` — lives in `src/extractor_identity.jl` since
# #409, so that `deps/build.jl` applies the same policy to the extractor's own
# build; it is still the policy every artifact key here uses.
